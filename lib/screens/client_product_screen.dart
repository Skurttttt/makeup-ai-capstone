  ...
  Widget _buildQuickStats(int total, int active, int lowStock, int outOfStock) {
    return Row(
      children: [
        ...
        const SizedBox(width: 8),
        Expanded(child: _buildStatCard(icon: Icons.remove_shopping_cart_rounded, label: 'Out of Stock', value: '$outOfStock', color: Colors.red.shade600)),
      ],
    );
  }
              ...
              child: SingleChildScrollView(
                ...
                Row(
                  ...
                  children: [
                    ...
                    const Spacer(),
                    _buildMiniActionButton(icon: Icons.reviews_rounded, color: Colors.purple.shade400, onPressed: () {
                      showModalBottomSheet(
                        context: context,
                        isScrollControlled: true,
                        backgroundColor: Colors.transparent,
                        builder: (_) => _SellerReviewsSheet(product: product),
                      );
                    }),
...
// ═══════════════════════════════════════════════════════════════════════════
// Seller reviews sheet – client views and replies to customer reviews
// ═══════════════════════════════════════════════════════════════════════════
class _SellerReviewsSheet extends StatefulWidget {
  final Map<String, dynamic> product;
  const _SellerReviewsSheet({required this.product});

  @override
  State<_SellerReviewsSheet> createState() => _SellerReviewsSheetState();
}

class _SellerReviewsSheetState extends State<_SellerReviewsSheet> {
  static const _pink = Color(0xFFFF4D8C);
  ...
  Future<List<Map<String, dynamic>>> _fetch() async {
    try {
      final rows = await Supabase.instance.client
          ...
          .select(
              'id, rating, comment, created_at, buyer_id, seller_reply, seller_replied_at, accounts(full_name, first_name)')
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child:
                        const Icon(Icons.reviews_rounded, color: _pink, size: 20),
                  ),
              ...
              child: FutureBuilder<List<Map<String, dynamic>>>(
                ...
                builder: (_, snap) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (_, i) => _ReviewReplyCard(
                      review: reviews[i],
                      onReplySaved: _reload,
                    ),
                  );
                },
              ),
...
// ── single review card with inline reply for the seller ─────────────────────
class _ReviewReplyCard extends StatefulWidget {
  final Map<String, dynamic> review;
  final VoidCallback onReplySaved;
  const _ReviewReplyCard(
      {required this.review, required this.onReplySaved});

  @override
  State<_ReviewReplyCard> createState() => _ReviewReplyCardState();
}

class _ReviewReplyCardState extends State<_ReviewReplyCard> {
  static const _pink = Color(0xFFFF4D8C);
  ...
  void initState() {
    ...
    _ctrl = TextEditingController(
        text: widget.review['seller_reply']?.toString() ?? '');
  }
  ...
  Future<void> _save() async {
    ...
    try {
      await Supabase.instance.client
          .from('product_reviews')
          .update({
            'seller_reply': text.isEmpty ? null : text,
            'seller_replied_at':
                text.isEmpty ? null : DateTime.now().toIso8601String(),
          })
          .eq('id', widget.review['id'].toString());
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('Could not save reply: $e'),
  ...
  Widget build(BuildContext context) {
    ...
    final accountJoin = widget.review['accounts'] as Map?;
    final buyerName = accountJoin?['full_name']?.toString() ??
        accountJoin?['first_name']?.toString() ??
        'Customer';
    final existingReply = widget.review['seller_reply']?.toString();
    final hasReply = existingReply != null && existingReply.isNotEmpty;
        ...
        children: [
          ...
          Container(
            decoration: BoxDecoration(
              ...
              border: const Border(
                  top: BorderSide(color: Color(0xFFFFD6E8))),
            ),
            padding: const EdgeInsets.all(12),
            child: _editing
                ? Column(
